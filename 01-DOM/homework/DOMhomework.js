...
function buildToDo(todo, index) {
  ...
  // ToDo {description: "string con la desc", complete: false}
  // Index 4

  // 1-2
  let toDoShell = document.createElement("div"); // {<div>   <span>     </div>}

  // toDoShell.className = "toDoShell"
  toDoShell.setAttribute("class", "toDoShell");

  // 3-4-5-6
  let toDoText = document.createElement("span"); // <span>
  toDoText.innerHTML = todo.description;
  toDoText.setAttribute("id", index);
  if (todo.complete) {
    toDoText.setAttribute("class", "completeText");
  }
  // ---- CONSIGNA COMPLETE TODO
  toDoText.addEventListener("click", completeToDo);
  // ---- CONSIGNA COMPLETE TODO FIN
  // 7
...
function addToDo() {
  ...
  if (input.value.trim() === "") return;
    ...
    // 1
    let tarea = new ToDo(input.value); // ToDo {description: "comprar", complete: false}
    // 2
    toDoItems.push(tarea);
    console.log("soy el input", input.value);
    // 3
    input.value = "";
    // 4
    displayToDos();
